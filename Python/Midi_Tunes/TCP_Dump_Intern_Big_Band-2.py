...
# To add track you can copy the first track (track = midi.Track() pattern.append(track)) an change the number behind.
    ...
    while line:
		...
		note  = MAX_OCTAVE
            ...
            track.append(midi.NoteOnEvent(tick=200, velocity=127, pitch=50))
            track.append(midi.NoteOffEvent(tick=200, pitch=50))
            track.append(midi.ProgramChangeEvent(data=[115]))
            
            track2.append(midi.NoteOnEvent(tick=50, velocity=127, pitch=midi.C_3))
            track2.append(midi.NoteOffEvent(tick=50, pitch=midi.C_3))
            track2.append(midi.ProgramChangeEvent(data=[120]))
            
            track3.append(midi.NoteOnEvent(tick=100, velocity=50, pitch=note))
            track3.append(midi.NoteOffEvent(tick=100, pitch=note))
            track3.append(midi.ProgramChangeEvent(data=[0]))
            
            track4.append(midi.NoteOnEvent(tick=50, velocity=50, pitch=note))
            track4.append(midi.NoteOffEvent(tick=100, pitch=note))
            track4.append(midi.ProgramChangeEvent(data=[27]))
            
            track5.append(midi.NoteOnEvent(tick=100, velocity=100, pitch=note))
            track5.append(midi.NoteOffEvent(tick=000, pitch=note))
            track5.append(midi.ProgramChangeEvent(data=[53]))

	    track6.append(midi.NoteOnEvent(tick=50, velocity=50, pitch=note))
            track6.append(midi.NoteOffEvent(tick=100, pitch=note))
	    track6.append(midi.ProgramChangeEvent(data=[32]))

	    track7.append(midi.NoteOnEvent(tick=0, velocity=75, pitch=note))
            track7.append(midi.NoteOffEvent(tick=25, pitch=note))
